#!/usr/bin/env npx tsx
/**
 * 우리부고 파트너 로고 + 거래처 CI 를 소스 Supabase Storage → 운영으로 복사
 *
 * 필수 env (.env.local):
 ...
 *
 * 실행:
 *   npm run sync-partner-client-assets
 */
...
async function main() {
  ...
  const { storageObjects } = await generateUribugoClientsTemplatesSql({
    partnerSubdomain: PARTNER_SUBDOMAIN,
    targetSupabaseUrl: targetUrl,
  });

  if (!storageObjects.length) {
    console.log("복사할 파트너/거래처 Storage 객체가 없습니다.");
    return;
  }
