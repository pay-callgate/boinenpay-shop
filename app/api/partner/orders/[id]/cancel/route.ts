...
export async function POST(
  ...
  try {
    ...
    if (!admin) {
      logger.warn("[API:PartnerOrderCancel] 파트너 권한 없음", {
        action: "partner_order_cancel_forbidden",
    ...
    logger.info("[API:PartnerOrderCancel] 요청 수신", {
      action: "partner_order_cancel_post",
      userId: session.user.id,
    ...
    if (!result.ok) {
      logger.warn("[API:PartnerOrderCancel] 처리 실패", {
        action: "partner_order_cancel_failed",
        userId: session.user.id,
        data: {
          orderId,
          code: result.code,
    ...
    logger.info("[API:PartnerOrderCancel] 성공", {
      action: "partner_order_cancel_ok",
      userId: session.user.id,
